...
model_22 = Sequential([
    Conv2D(128, (3, 20), activation='relu', border_mode='valid', kernel_regularizer=kernel_regularizer, input_shape=(1107, 20, 1)),
    Conv2D(128, (3, 1), activation='relu', border_mode='valid', kernel_regularizer=kernel_regularizer, input_shape=(1107, 20, 1)),
    Flatten(),
    Dense(256),
    #Reshape((-1, 128)),
...
print ("model_22 BUILT")

model_22.compile(loss='categorical_crossentropy', optimizer="adam", metrics=['accuracy'])
print ("model_22 COMPILED")


checkpoint = ModelCheckpoint(filepath='/home/hpc_lkpiel/models/model_22.hdf5', monitor='val_loss', save_best_only=True)

history = model_22.fit(x=train_data_padded,
...
np.save('../history/history_model_22.npy', history.history)
modelHistory = np.load('../history/history_model_22.npy').item()

print ("HISTORY: ")
print (modelHistory)
model_22.load_weights('/home/hpc_lkpiel/models/model_22.hdf5')

valResult = model_22.evaluate(val_data_padded, val_labels_age_group)
testResult = model_22.evaluate(test_data_padded, test_labels_age_group)

file = open("results.txt","a")
file.write("\nmodel_22 VAL: " + str(valResult) + " TEST: " + str(testResult))
file.close()
print ("WROTE TO FILE")
