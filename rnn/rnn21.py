...
print ("model_21 BUILT")

model_21.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])
print ("model_21 COMPILED")


checkpoint = ModelCheckpoint(filepath='/home/hpc_lkpiel/models/model_21.hdf5', monitor='val_loss', save_best_only=True)

history = model_21.fit(x=train_data_padded,
...
np.save('../history/history_model_21.npy', history.history)
modelHistory = np.load('../history/history_model_21.npy').item()

print ("HISTORY: ")
print (modelHistory)
model_21.load_weights('/home/hpc_lkpiel/models/model_21.hdf5')

valResult = model_21.evaluate(val_data_padded, val_labels_age_group)
testResult = model_21.evaluate(test_data_padded, test_labels_age_group)

file = open("results.txt","a")
file.write("\nmodel_21 VAL: " + str(valResult) + " TEST: " + str(testResult))
file.close()
print ("WROTE TO FILE")
