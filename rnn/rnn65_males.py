...
train_labels_age_group_males = onehot(train_data_i_vectors[train_data_i_vectors['Gender'] == 0]['AgeGroup'])
...
train_labels_age_group_males = np.load("../dnn/dnn_predicted_labels.npy")[np.array(trainMaleIndexes)]
...
val_labels_age_group_males = onehot(val_data_i_vectors[val_data_i_vectors['Gender'] == 0]['AgeGroup'])
...
test_labels_age_group_males = onehot(test_data_i_vectors[test_data_i_vectors['Gender'] == 0]['AgeGroup'])
...
print (model_65_males.summary)
print ("model_65_males BUILT")

model_65_males.compile(loss='categorical_crossentropy', optimizer="adam", metrics=['accuracy'])
print ("model_65_males COMPILED")


checkpoint = ModelCheckpoint(filepath='/home/hpc_lkpiel/models/model_65_males.hdf5', monitor='val_acc', save_best_only=True)

history = model_65_males.fit(x=train_data_padded,
        y=train_labels_age_group_males,
        validation_data=(val_data_padded, val_labels_age_group_males),
        epochs=40,
        verbose=1,
        batch_size=128,
...
np.save('../history/history_model_65_males.npy', history.history)
modelHistory = np.load('../history/history_model_65_males.npy').item()
...
val_predictions = model_65_males.predict(val_data_padded)
print ("VAL PREDICTED")

test_predictions = model_65_males.predict(test_data_padded)
print ("TEST PREDICTED")

np.save('/home/hpc_lkpiel/predictions/val/model_65_males.npy', val_predictions)
print ("VAL SAVED")

np.save('/home/hpc_lkpiel/predictions/test/model_65_males.npy', test_predictions)
print ("TEST SAVED")
