...
input_layer_2 = Input(shape=(1107, 20, 1), name="lstm_input")
y = Conv2D(128, (3, 20), activation='relu', kernel_regularizer=kernel_regularizer,  border_mode='valid')(input_layer)
y = Conv2D(128, (5, 1), strides=(3,1), activation='relu', kernel_regularizer=kernel_regularizer, border_mode='valid')(y)
y = Conv2D(128, (5, 1), strides=(3,1), activation='relu', kernel_regularizer=kernel_regularizer, border_mode='valid')(y)
y = Conv2D(128, (5, 1), strides=(3,1), activation='relu', kernel_regularizer=kernel_regularizer, border_mode='valid')(y)
y = Conv2D(128, (5, 1), strides=(3,1), activation='relu', kernel_regularizer=kernel_regularizer, border_mode='valid')(y)
y = Reshape((-1, 128))(y)
...
print ("model_62 BUILT")
model_62 = Model(inputs=[input_layer, input_layer_2], outputs=output_layer_1)

model_62.compile(loss='categorical_crossentropy',
                    optimizer="adam",
                    metrics=['accuracy'])
...
history = model_62.fit([train_data_padded, train_data_padded],
    ...
    validation_data=([val_data_padded, val_data_padded], [val_labels_age_group]),
    epochs=40,
    verbose=1,
    batch_size=128,
...
np.save('../history/history_model_62.npy', history.history)
modelHistory = np.load('../history/history_model_62.npy').item()
...
test_predictions = model_62.predict([val_data_padded, val_data_padded])
print ("TEST PREDICTED")


np.save('/home/hpc_lkpiel/predictions/val/model_62.npy', val_predictions)
print ("VAL SAVED")

np.save('/home/hpc_lkpiel/predictions/test/model_62.npy', test_predictions)
print ("TEST SAVED")


valResult = model_62.evaluate([val_data_padded, val_i_vectors], val_labels_age_group)
testResult = model_62.evaluate([test_data_padded, test_i_vectors], test_labels_age_group)

file = open("results.txt","a")
file.write("\nmodel_62 VAL: " + str(valResult) + " TEST: " + str(testResult))
file.close()
print ("WROTE TO FILE model_62")
