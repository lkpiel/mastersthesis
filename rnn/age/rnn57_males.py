...
trainMaleIndexes = train_data.index[train_data['Gender'] == 0].tolist()
valMaleIndexes = val_data.index[val_data['Gender'] == 0].tolist()
testMaleIndexes = test_data.index[test_data['Gender'] == 0].tolist()
...
print ("model_57 BUILT")
model_57 = Model(inputs=[input_layer, input_layer_2], outputs=output_layer_1)

model_57.compile(loss='categorical_crossentropy',
                    optimizer=SGD(0.01),
                    metrics=['accuracy'])

print ("model_57 COMPILED")


checkpoint = ModelCheckpoint(filepath='/home/hpc_lkpiel/models/model_57_males.hdf5', monitor='val_acc', save_best_only=True)


history = model_57.fit([train_data_padded, train_i_vectors_males],
    [train_labels_males],
    validation_data=([val_data_padded, val_i_vectors_males], [val_labels_males]),
    epochs=80,
    verbose=1,
    batch_size=128,
...
np.save('../history/history_model_57_males.npy', history.history)
modelHistory = np.load('../history/history_model_57_males.npy').item()

print ("HISTORY: ")
print (modelHistory)


model_57.load_weights('/home/hpc_lkpiel/models/model_57_males.hdf5')

val_predictions = model_57.predict([val_data_padded, val_i_vectors_males])
print ("VAL PREDICTED")

test_predictions = model_57.predict([test_data_padded, test_i_vectors_males])
print ("TEST PREDICTED")


np.save('/home/hpc_lkpiel/predictions/val/model_57_males.npy', val_predictions)
print ("VAL SAVED")

np.save('/home/hpc_lkpiel/predictions/test/model_57_males.npy', test_predictions)
print ("TEST SAVED")
'''
valResult = model_57.evaluate([val_data_padded, val_i_vectors], val_labels_age_group)
testResult = model_57.evaluate([test_data_padded, test_i_vectors], test_labels_age_group)

file = open("results.txt","a")
file.write("\nmodel_57 VAL: " + str(valResult) + " TEST: " + str(testResult))
file.close()
print ("WROTE TO FILE model_57")
'''
########################################