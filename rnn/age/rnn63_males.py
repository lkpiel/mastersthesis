...
trainMaleIndexes = train_data_i_vectors.index[train_data_i_vectors['Gender'] == 0].tolist()
train_data_i_vectors = train_data_i_vectors[train_data_i_vectors['Gender'] == 0]
train_labels_age_group = onehot(train_data_i_vectors['AgeGroup'])
#trainMaleIndexes = train_data_i_vectors.index[train_data_i_vectors['Gender'] == 1].tolist()
...
valMaleIndexes = val_data_i_vectors.index[val_data_i_vectors['Gender'] == 0].tolist()
val_data_i_vectors = val_data_i_vectors[val_data_i_vectors['Gender'] == 0]
val_labels_age_group = onehot(val_data_i_vectors['AgeGroup'])
#valMaleIndexes = val_data_i_vectors.index[val_data_i_vectors['Gender'] == 1].tolist()
...
testMaleIndexes = test_data_i_vectors.index[test_data_i_vectors['Gender'] == 0].tolist()
test_data_i_vectors = test_data_i_vectors[test_data_i_vectors['Gender'] == 0]
test_labels_age_group = onehot(test_data_i_vectors['AgeGroup'])


train_data_i_vectors = train_data_i_vectors.iloc[:, 5:].as_matrix()
val_data_i_vectors = val_data_i_vectors.iloc[:, 5:].as_matrix()
test_data_i_vectors = test_data_i_vectors.iloc[:, 5:].as_matrix()

print (train_data_i_vectors.shape)
...
print ("model_63_males BUILT")
model_63_males = Model(inputs=[input_layer, input_layer_2], outputs=output_layer_1)

model_63_males.compile(loss='categorical_crossentropy',
                    optimizer="sgd",
                    metrics=['accuracy'])

print ("model_63_males COMPILED")


checkpoint = ModelCheckpoint(filepath='/home/hpc_lkpiel/models/model_63_males.hdf5', monitor='val_acc', save_best_only=True)


history = model_63_males.fit([train_data_padded, train_data_i_vectors],
    [train_labels_age_group],
    validation_data=([val_data_padded, val_data_i_vectors], [val_labels_age_group]),
    epochs=20,
    verbose=1,
    batch_size=128,
...
np.save('../history/history_model_63_males.npy', history.history)
modelHistory = np.load('../history/history_model_63_males.npy').item()

print ("HISTORY: ")
print (modelHistory)

model_63_males.load_weights('/home/hpc_lkpiel/models/model_63_males.hdf5')


val_predictions = model_63_males.predict([val_data_padded, val_data_i_vectors])
print ("VAL PREDICTED")

test_predictions = model_63_males.predict([test_data_padded, test_data_i_vectors])
print ("TEST PREDICTED")


np.save('/home/hpc_lkpiel/predictions/val/model_63_males.npy', val_predictions)
print ("VAL SAVED")

np.save('/home/hpc_lkpiel/predictions/test/model_63_males.npy', test_predictions)
print ("TEST SAVED")


valResult = model_63_males.evaluate([val_data_padded, val__data_i_vectors], val_labels_age_group)
testResult = model_63_males.evaluate([test_data_padded, test_data_i_vectors], test_labels_age_group)

file = open("results.txt","a")
file.write("\nmodel_63_males VAL: " + str(valResult) + " TEST: " + str(testResult))
file.close()
print ("WROTE TO FILE model_63_males")
