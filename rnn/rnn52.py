...
for i in range(train_labels_age.size):
    x.append(train_labels_age[i] + random.uniform(-0.8, 0.8))
train_labels_age = np.array(x)
train_labels_age = np.save("./randomized_train_labels_0_8", train_labels_age)
...
print ("model_52 BUILT")

model_52.compile(loss='mse', optimizer=SGD(0.01), metrics=[age_group_accuracy])
print ("model_52 COMPILED")

checkpoint = ModelCheckpoint(filepath='/home/hpc_lkpiel/models/model_52.hdf5', monitor='val_age_group_accuracy', save_best_only=True)

history = model_52.fit(x=train_data_padded,
        y=train_labels_age,
...
np.save('../history/history_model_52.npy', history.history)
modelHistory = np.load('../history/history_model_52.npy').item()

print ("HISTORY: ")
print (modelHistory)
model_52.load_weights('/home/hpc_lkpiel/models/model_52.hdf5')

valResult = model_52.evaluate(val_data_padded, val_labels_age)
testResult = model_52.evaluate(test_data_padded, test_labels_age)

file = open("results.txt","a")
file.write("\nmodel_52 VAL: " + str(valResult) + " TEST: " + str(testResult))
file.close()
print ("WROTE TO FILE model_52")
