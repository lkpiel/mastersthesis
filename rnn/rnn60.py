...
print ("model_60 BUILT")

model_60.compile(loss='categorical_crossentropy', optimizer='rmsprop', metrics=['accuracy'])
print ("model_60 COMPILED")


checkpoint = ModelCheckpoint(filepath='/home/hpc_lkpiel/models/model_60.hdf5', monitor='val_loss', save_best_only=True)

history = model_60.fit(x=train_data_padded,
...
np.save('../history/history_model_60.npy', history.history)
modelHistory = np.load('../history/history_model_60.npy').item()

print ("HISTORY: ")
print (modelHistory)
model_60.load_weights('/home/hpc_lkpiel/models/model_60.hdf5')

valResult = model_60.evaluate(val_data_padded, val_labels_age_group)
testResult = model_60.evaluate(test_data_padded, test_labels_age_group)

file = open("results.txt","a")
file.write("\nmodel_60 VAL: " + str(valResult) + " TEST: " + str(testResult))
file.close()
print ("WROTE TO FILE")
