...
model_24 = Sequential([
    ...
    Conv2D(128, (3, 1), strides=(1,1), activation='relu', border_mode='valid'),
    Conv2D(128, (3, 1), strides=(3,1), activation='relu', border_mode='valid'),
    Reshape((-1, 128)),
...
print ("model_24 BUILT")

model_24.compile(loss='categorical_crossentropy', optimizer="adam", metrics=['accuracy'])
print ("model_24 COMPILED")


checkpoint = ModelCheckpoint(filepath='/home/hpc_lkpiel/models/model_24.hdf5', monitor='val_loss', save_best_only=True)

history = model_24.fit(x=train_data_padded,
...
np.save('../history/history_model_24.npy', history.history)
modelHistory = np.load('../history/history_model_24.npy').item()

print ("HISTORY: ")
print (modelHistory)
model_24.load_weights('/home/hpc_lkpiel/models/model_24.hdf5')

valResult = model_24.evaluate(val_data_padded, val_labels_age_group)
testResult = model_24.evaluate(test_data_padded, test_labels_age_group)

file = open("results.txt","a")
file.write("\nmodel_24 VAL: " + str(valResult) + " TEST: " + str(testResult))
file.close()
print ("WROTE TO FILE")
