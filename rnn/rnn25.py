...
model_25 = Sequential([
    Conv2D(256, (3, 20), activation='relu', border_mode='valid', input_shape=(1107, 20, 1)),
    Conv2D(256, (5, 1),  activation='relu', border_mode='valid'),
    Conv2D(256, (5, 1), strides=(3,1), activation='relu', border_mode='valid'),
    Conv2D(256, (5, 1), strides=(3,1), activation='relu', border_mode='valid'),
    Reshape((-1, 64)),
...
print ("model_25 BUILT")

model_25.compile(loss='categorical_crossentropy', optimizer="adam", metrics=['accuracy'])
print ("model_25 COMPILED")


checkpoint = ModelCheckpoint(filepath='/home/hpc_lkpiel/models/model_25.hdf5', monitor='val_loss', save_best_only=True)

history = model_25.fit(x=train_data_padded,
...
np.save('../history/history_model_25.npy', history.history)
modelHistory = np.load('../history/history_model_25.npy').item()

print ("HISTORY: ")
print (modelHistory)
model_25.load_weights('/home/hpc_lkpiel/models/model_25.hdf5')

valResult = model_25.evaluate(val_data_padded, val_labels_age_group)
testResult = model_25.evaluate(test_data_padded, test_labels_age_group)

file = open("results.txt","a")
file.write("\nmodel_25 VAL: " + str(valResult) + " TEST: " + str(testResult))
file.close()
print ("WROTE TO FILE")
