...
dnn_7 = Model(inputs=input_layer, outputs=[output_layer_1, output_layer_2])
dnn_7.compile(loss={'gender_output':'categorical_crossentropy', 'age_output':'mse'},
                    optimizer=SGD(0.01),
                    metrics={'gender_output':'accuracy','age_output':age_group_accuracy})
dnn_7.load_weights('/home/hpc_lkpiel/models/dnn_7_gender.hdf5')
print ("dnn_7 WEIGHTS LOADED")
...
model_75_females_original = Sequential([
...
model_75_females_original.compile(loss='categorical_crossentropy', optimizer=SGD(0.01), metrics=['accuracy'])
model_75_females_original.load_weights('/home/hpc_lkpiel/models/model_69.hdf5')
print ("model_75 WEIGHTS LOADED")
...
def combinePredictions(i_vectors, sequence_data):
    ...
    for i in range(0, len(i_vectors)):
        ...
        if (np.argmax(gender_prediction) == 1):
            age_group_prediction = 0.2 * np.squeeze(formatAgePredictions(dnn_6_females_original.predict(vector)[1])) + 0.1 *  np.squeeze(dnn_7.predict(vector)[1]) + 0.4 *  np.squeeze(model_63_females_original.predict([padded_data, vector])) + 0.1 *  np.squeeze(model_75_females_original.predict(padded_data)) + 0.2 *  np.squeeze(model_65_females_original.predict(padded_data))
        else:
            age_group_prediction =  np.squeeze(formatAgePredictions(dnn_2_males.predict(vector)))
...
def combinePredictionsWeighted(i_vectors, sequence_data):
    ...
    for i in range(0, len(i_vectors)):
        ...
        gender_prediction = model_90.predict([padded_data, vector])
        age_group_prediction = np.squeeze(gender_prediction)[0]*np.squeeze(formatAgePredictions(dnn_6_males.predict(vector)[1])) + np.squeeze(gender_prediction)[1]*np.squeeze(formatAgePredictions(dnn_6_females.predict(vector)[1]))
...
'''
'''
np.save('/home/hpc_lkpiel/predictions/val/combined_prediction_3.npy', combined_predictions_val)
print ("VAL SAVED")
'''
np.save('/home/hpc_lkpiel/predictions/test/combined_prediction_3.npy', combined_predictions_test)
print ("TEST SAVED")
'''
np.save('/home/hpc_lkpiel/predictions/val/combined_prediction_weighted_3.npy', combined_predictions_weighted_val)
print ("VAL SAVED")

np.save('/home/hpc_lkpiel/predictions/test/combined_prediction_weighted_3.npy', combined_predictions_weighted_test)
print ("TEST SAVED")
'''
########################################