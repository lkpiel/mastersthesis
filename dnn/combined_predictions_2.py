...
output_layer_1 = Dense(2, activation='softmax')(z)

print ("model_90 BUILT")
model_90 = Model(inputs=[input_layer, input_layer_2], outputs=output_layer_1)

model_90.compile(loss='categorical_crossentropy',
                    optimizer=SGD(0.01),
                    metrics=['accuracy'])
model_90.load_weights('/home/hpc_lkpiel/models/model_90.hdf5')


print ("model_90 MODEL COMPILED")
...
dnn_6_males = Model(inputs=input_layer, outputs=[output_layer_1, output_layer_2])

dnn_6_males.compile(loss={'group_output':'categorical_crossentropy', 'age_output':'mse'},
                    optimizer="sgd",
                    metrics={'group_output':'accuracy','age_output':age_group_accuracy})
print ("dnn_6_males MODEL COMPILED")
dnn_6_males.load_weights("/home/hpc_lkpiel/models/dnn_6_males.hdf5")
...
dnn_6_females = Model(inputs=input_layer, outputs=[output_layer_1, output_layer_2])

dnn_6_females.compile(loss={'group_output':'categorical_crossentropy', 'age_output':'mse'},
                    optimizer="sgd",
                    metrics={'group_output':'accuracy','age_output':age_group_accuracy})
print ("dnn_6_females MODEL COMPILED")
dnn_6_females.load_weights("/home/hpc_lkpiel/models/dnn_6.hdf5")
...
def combinePredictions(i_vectors, sequence_data):
    ...
    for i in range(0, len(i_vectors)):
        ...
        if (np.argmax(gender_prediction) == 1):
            age_group_prediction = formatAgePredictions(dnn_6_females.predict(vector)[1])
        else:
            age_group_prediction = formatAgePredictions(dnn_6_males.predict(vector)[1])
        print (age_group_prediction)
        age_group_prediction = np.squeeze(age_group_prediction, axis=0)
...
np.save('/home/hpc_lkpiel/predictions/test/combined_prediction_weighted_2.npy', combined_predictions_weighted_test)
print ("TEST SAVED")
